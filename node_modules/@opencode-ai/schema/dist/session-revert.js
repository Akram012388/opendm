...
const PersistedCurrent = Revert.pipe(Schema.decodeTo(Schema.Struct({ source: Schema.tag("current"), revert: Schema.toType(Revert) }), SchemaTransformation.transform({
    decode: (revert) => ({
        source: "current",
        revert,
    }),
...
const PersistedLegacy = RevertV1.pipe(Schema.decodeTo(Schema.Struct({ source: Schema.tag("legacy"), revert: Schema.toType(RevertV1) }), SchemaTransformation.transform({
    decode: (revert) => ({
        source: "legacy",
        revert,
    }),
...
export const PersistedRevert = Schema.Union([PersistedCurrent, PersistedLegacy]).pipe(Schema.toTaggedUnion("source"), Schema.decodeTo(Schema.toType(Revert), SchemaTransformation.transform({
    decode: (persisted) => {
